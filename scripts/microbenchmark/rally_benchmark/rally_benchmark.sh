...
function run_once {
    ...
    for j in $(seq 1 1 $test_times); do
		...
		docker run --rm --net=host -v $AUTOCANCEL_HOME/scripts/data/rally_home:/home/rally/.rally -t rally_exp:v1.0 \
			esrally race --track-revision=8.13 --track=random_vector --pipeline=benchmark-only --target-hosts=127.0.0.1:9200 --report-file=/home/rally/.rally/report-${5}-${BENCHMARK_START_TIME}.csv --report-format=csv \
