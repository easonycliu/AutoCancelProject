...
function run_once {
    ...
    mv $AUTOCANCEL_HOME/scripts/logs/$START_DATE/${MICROBENCHMARK}_${START_TIME}/${4}.csv $AUTOCANCEL_HOME/scripts/logs/$START_DATE/${MICROBENCHMARK}_${START_TIME}/${6}_${4}_${7}.csv
    mv $AUTOCANCEL_HOME/scripts/logs/$START_DATE/${MICROBENCHMARK}_${START_TIME}/${4}_latency.csv $AUTOCANCEL_HOME/scripts/logs/$START_DATE/${MICROBENCHMARK}_${START_TIME}/${6}_${4}_${7}_latency.csv
    mv $AUTOCANCEL_HOME/scripts/logs/$START_DATE/${MICROBENCHMARK}_${START_TIME}/${4}_throughput.csv $AUTOCANCEL_HOME/scripts/logs/$START_DATE/${MICROBENCHMARK}_${START_TIME}/${6}_${4}_${7}_throughput.csv

	bash -c "$env_args docker compose -f $AUTOCANCEL_HOME/scripts/microbenchmark/interval_sensitivity/${app_exp}_docker_config.yml down"
}

function run_case {
    run_once base_policy false true base_wo_predict $client_num $1 $2
    sleep 10

    run_once base_policy true true base_w_predict $client_num $1 $2
    sleep 10

    run_once multi_objective_policy false true moo_wo_predict $client_num $1 $2
    sleep 10

    run_once multi_objective_policy true true moo_w_predict $client_num $1 $2
    sleep 10

    run_once multi_objective_policy true false wo_cancel $client_num $1 $2
    sleep 10

    run_once multi_objective_policy true false normal $client_num $1 $2
    sleep 10
}
