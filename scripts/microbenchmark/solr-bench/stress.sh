...
AUTOCANCEL_HOME=$(git rev-parse --show-toplevel)
...
then
     while read i; do
         ...
         if [ ! -d $_LOCALREPO ]; then
             ln -s $AUTOCANCEL_HOME/solr $_LOCALREPO
         fi
         cd $_LOCALREPO
...
then
  SOLR_OPTS=$EXTRA_JVM_ARGS java -Xmx12g -cp $BASEDIR/target/org.apache.solr.benchmarks-${SOLR_BENCH_VERSION}-jar-with-dependencies.jar:. \
   StressMain -f $CONFIGFILE -c $COMMIT
  java_exit_code=$?
else
  SOLR_OPTS=$EXTRA_JVM_ARGS java -Xmx12g -cp ${SOLR_BENCH_JAR}:. StressMain -f $CONFIGFILE -c $COMMIT
  java_exit_code=$?
fi
