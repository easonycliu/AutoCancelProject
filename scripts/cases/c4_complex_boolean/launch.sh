...
function run_once {
	...
	bash -c "$env_args docker compose -f $AUTOCANCEL_HOME/scripts/cases/c4_complex_boolean/docker_config.yml up &"
    sleep 90
    ...
    docker run --rm --net=host -v $AUTOCANCEL_HOME/autocancel_exp/elasticsearch_exp:/root -w /root easonliu12138/es_py_env:v1.1 /root/scripts/multiclient_complex_boolean.sh $5 ${4}_${START_TIME} $4
    sleep 10
