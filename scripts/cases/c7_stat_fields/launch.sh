...
function run_once {
    env_args="DEFAULT_POLICY=$1 PREDICT_PROGRESS=$2 CANCEL_ENABLE=$3 AUTOCANCEL_LOG=$4"
	bash -c "$env_args docker compose -f $AUTOCANCEL_HOME/scripts/cases/c7_stat_fields/docker_config.yml down"

    docker stop single_node || true
    docker rm single_node || true

	bash -c "$env_args docker compose -f $AUTOCANCEL_HOME/scripts/cases/c7_stat_fields/docker_config.yml up &"
    sleep 60
