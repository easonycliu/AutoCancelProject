...
def get_recover_time(log_list):
    ...
    recover_list = [recover_time_stabilizer.get(x[3] == "true") for x in log_list]
...
if __name__ == "__main__":
    ...
    for log_dir in log_dirs:
        ...
        for mode in experiment_modes:
            lib_log_df = pd.read_csv(os.path.join(os.getcwd(), "logs", "{}_{}_{}".format(year, month, day), log_dir, "{}.csv".format(mode)), dtype={'Cancel': str, "Recover": str})
            throughput_log_df = pd.read_csv(os.path.join(os.getcwd(), "logs", "{}_{}_{}".format(year, month, day), log_dir, "{}_throughput.csv".format(mode)))
            latency_log_df = pd.read_csv(os.path.join(os.getcwd(), "logs", "{}_{}_{}".format(year, month, day), log_dir, "{}_latency.csv".format(mode)))
            ...
            avg_throughput_dict[mode].append(get_average_throughput(throughput_log_df.values.squeeze()))
            avg_latency_dict[mode].append(get_average_latency(latency_log_df.values.squeeze()))
            p99_latency_dict[mode].append(get_p99_latency(latency_log_df.values.squeeze()))
            cancel_time_dict[mode].append(0 if mode == "wo_cancel" else get_cancel_time(lib_log_list))
            recover_time_dict[mode].append(get_recover_time(lib_log_list))
            draw_throught(lib_log_list, os.path.join(os.getcwd(), "logs", "{}_{}_{}".format(year, month, day), log_dir, "{}.jpg".format(mode)))
            
    for mode in experiment_modes:
        print("Mode: {}, Avg Throughput: {}, Avg Latency: {}, P99 Latency: {}, Cancel Time: {}, Recover Time: {}".format(mode, np.mean(avg_throughput_dict[mode]), np.mean(avg_latency_dict[mode]), np.mean(p99_latency_dict[mode]), np.mean(cancel_time_dict[mode]), np.mean(recover_time_dict[mode])))
